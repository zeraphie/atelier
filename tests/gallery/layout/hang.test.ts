...
import { roomAt, workAt } from "../../../src/gallery/layout/targets.js";
import type { Room } from "../../../src/gallery/works.js";
import { SPACING, room, work } from "../../fixtures.js";
import { centre, distance } from "../../../src/geometry.js";
...
describe("hangGallery", () => {
  ...
  test("the entrance is in the first room's bottom wall, in the metre a third along from the end nearest the next room", () => {
    ...
    expect(distance(entrance.gap.a, entrance.gap.b)).toBe(20);
    // The next room is to the right, so in the metre a third of the way from the right end.
    expect(centre(entrance.gap).x).toBe(250);
  });
  ...
  test("walls are each edge once, with the doorways cut out a wall wider", () => {
    ...
    expect(100 - shared.reduce((sum, w) => sum + distance(w.a, w.b), 0)).toBe(20 + 4);
    const bottomOfA = walls.filter((w) => w.a.y === 100 && w.b.y === 100 && w.b.x <= 100);
    expect(bottomOfA.reduce((sum, w) => sum + distance(w.a, w.b), 0)).toBe(100 - 24);
  });
...
describe("doorways of drawn rooms", () => {
  ...
  test("a drawn room's door is in the middle metre of the wall it shares", () => {
    ...
    expect(centre(toC.gap).x).toBe(450);
    expect(distance(toC.gap.a, toC.gap.b)).toBe(20);
  });
...
describe("doorways by hand", () => {
  ...
  test("a doorway put by hand holds when its edge lies on a wall the two rooms share, centred there", () => {
    ...
    expect(centre(doorways.find((d) => d.to === "c")!.gap)).toEqual({ x: 150, y: 200 });
  });
...
describe("ways outside and the entrance", () => {
  ...
  test("the entrance moves to a chosen outer edge of the first room, and a null takes it away", () => {
    ...
    expect(centre(moved.doorways[0]!.gap)).toEqual({ x: 0, y: 150 });
