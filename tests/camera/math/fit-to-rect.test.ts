...
describe("fitToRect", () => {
  test("a wide rect fits the width and centres vertically", () => {
    const camera = fitToRect(size, { left: 0, top: 0, right: 2000, bottom: 500 }, limits);
  ...
  test("a tall rect fits the height and centres horizontally", () => {
    const camera = fitToRect(size, { left: 0, top: 0, right: 250, bottom: 1000 }, limits);
    expect(camera.zoom).toBe(0.5);
    expect(worldToScreen(camera, { x: 0, y: 0 })).toEqual({ x: 437.5, y: 0 });
  });

  test("padding keeps a clear margin on the tight axis", () => {
    const camera = fitToRect(size, { left: 0, top: 0, right: 2000, bottom: 500 }, limits, 100);
  ...
  test("a cap centres a small rect instead of blowing it up", () => {
    ...
    const camera = fitToRect(size, { left: 0, top: 0, right: 500, bottom: 250 }, limits, 0, 1);
    ...
    const wide = fitToRect(size, { left: 0, top: 0, right: 2000, bottom: 500 }, limits, 0, 1);
    expect(wide.zoom).toBe(0.5);
  });

  test("a rect not at the origin is still centred", () => {
    const camera = fitToRect(size, { left: 300, top: 200, right: 800, bottom: 450 }, limits);
    expect(camera.zoom).toBe(2);
    expect(worldToScreen(camera, { x: 550, y: 325 })).toEqual({ x: 500, y: 250 });
  });

  test("zoom is clamped to the limits", () => {
    const camera = fitToRect(size, { left: 0, top: 0, right: 10, bottom: 10 }, limits);
